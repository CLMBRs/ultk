...
class TestLanguage:
    ...
    def test_language_universe_check(self):
        with pytest.raises(ValueError):
            lang2 = Language([TestLanguage.exp, 
                              Expression(form="dog", meaning=Meaning(referents=[Referent("dog", {"phylum":"animal"})], universe=TestLanguage.uni2))])
    def test_language_degree(self):
        ...
        assert TestLanguage.lang.degree_property(isAnimal) == .5
