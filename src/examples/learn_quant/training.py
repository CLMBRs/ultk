...
class QuantifierDataset(Dataset):
    ...
    def __init__(self, expression: GrammaticalExpression, representation="one_hot", downsampling=True, generation_args=None):
        """
        Args:
            data (list or array-like): A collection of objects to be transformed into vector representations.
            metadata (list or array-like, optional): Additional information or labels tied to each data object.
...
class MV_LSTM(torch.nn.Module):
    ...
    def __init__(self,input_dim,seq_len, device="cpu"):
        ...
        self.l_lstm = torch.nn.LSTM(input_size = input_dim, 
...
class TransformerModel(nn.Module):
    def __init__(self, input_dim=5, seq_len=8, num_classes=2, d_model=32, nhead=4, num_encoder_layers=2, dim_feedforward=32, device="cpu"):
        super(TransformerModel, self).__init__()
        self.device = device
        
        self.embedding = nn.Linear(input_dim, d_model, device=device)
        
        self.positional_encoding = PositionalEncoding(d_model, max_len=seq_len, device=device)
        
        self.transformer = nn.Transformer(
            d_model=d_model, nhead=nhead, num_encoder_layers=num_encoder_layers, dim_feedforward=dim_feedforward, device=device
        )
        
        self.fc_out = nn.Linear(d_model, num_classes, device=device)  # Flatten transformer output to predict [0, 1] or [1, 0]
    ...
    def forward(self, x):
        ...
        self.src_mask = nn.Transformer.generate_square_subsequent_mask(x.size(1), device=torch.device(self.device))
        ...
        x = self.transformer.encoder(x, mask=self.src_mask)  # Shape: (seq_len, batch_size, d_model) # should be x = self.transformer.encoder(x)
