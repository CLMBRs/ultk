...
def wait_for_mlflow(max_retries=30, retry_delay=30):
    ...
    for attempt in range(max_retries):
        if is_mlflow_server_up(mlflow_tracking_uri):
            ...
            logger.warning(f"MLflow server is down. Retrying in {retry_delay} seconds (Attempt {attempt+1}/{max_retries}).")
...
class MLFlowConnectivityCallback(Callback):
    def __init__(self, retry_delay=30, max_retries=5):
        ...
        logger.info(f"MLFlowConnectivityCallback initialized: {self.mlflow_tracking_uri}")
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        ...
        if not is_mlflow_server_up(self.mlflow_tracking_uri):
            ...
            try:
                wait_for_mlflow(max_retries=self.max_retries, retry_delay=self.retry_delay)
