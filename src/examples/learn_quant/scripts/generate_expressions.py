...
def enumerate_quantifiers(depth: int, 
...
def generation_time_trial(quantifiers_grammar: QuantifierGrammar, 
                         cfg: DictConfig
                         ):
    
    time_log = pathlib.Path(cfg.output) / pathlib.Path("generated_expressions_time_log.csv")
    ...
    with open(time_log, "a+", newline='') as f:
        ...
        for m_size in range(1, cfg.universe.m_size+1):
            ...
            for depth in range(2, cfg.universe.depth+1):
                ...
                from pathlib import Path
                outpath = Path(cfg.output) / Path("M"+str(m_size)) / Path("X"+str(cfg.universe.x_size)) / Path(str("d"+str(depth))) / Path("generated_expressions.yml")
...
def generate_inclusive_expressions(quantifiers_grammar, cfg, save=True):
    ...
    for m_size in range(1, cfg.universe.m_size+1):
        ...
        print("")
        if save:
            save_inclusive_generation(expressions_by_meaning, master_universe, cfg.output, m_size, cfg.universe.x_size, cfg.universe.depth)
