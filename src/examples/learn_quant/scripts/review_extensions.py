...
def nested_model_redraw(df: pd.DataFrame, outdir: Path) -> None:
    ...
    print(f"n={int(LM.nobs)}  R^2: base={r2[0]:.3f}  L={r2[1]:.3f}  M={r2[2]:.3f}  LM={r2[3]:.3f}")
    print(f"add mono on L: dR2=+{uM:.3f} (p={p_addM:.1e});  add length on M: dR2=+{uL:.3f} (p={p_addL:.1e})")

    fig, axes = plt.subplots(1, 2, figsize=(13, 5))
    ax = axes[0]
    names = ["model type\nonly", "+ length\n(func_count, L)",
             "+ monotonicity\n(degree, M)", "length + mono\n(full, LM)"]
    ...
    ax.annotate("", xy=(2.85, r2[3] + 0.006), xytext=(1.15, r2[1] + 0.006),
                arrowprops=dict(arrowstyle="->", color="#2e6b40", lw=1.6,
                                connectionstyle="arc3,rad=-0.3"))
    ax.text(1.62, r2[3] + 0.016, f"L → LM: add monotonicity\nΔR² = +{uM:.3f}   (F-test p = {p_addM:.0e})",
            ha="center", fontsize=9, color="#2e6b40")
    ax.annotate("", xy=(3.0, r2[3] - 0.02), xytext=(2.35, r2[2] - 0.02),
                arrowprops=dict(arrowstyle="->", color="#31517e", lw=1.6,
                                connectionstyle="arc3,rad=0.25"))
    ax.text(2.72, r2[1] + 0.011, f"M → LM: add length\nΔR² = +{uL:.3f}\n(p = {p_addL:.0e})",
            ha="center", fontsize=9, color="#31517e")
    ax.set_xticks(range(4))
    ax.set_xticklabels(names, fontsize=10)
    ax.set_ylabel("R²  (Validation Loss AUC)", fontweight="bold")
    ax.set_ylim(0, r2[3] + 0.046)
    ax.set_title("Nested OLS models — length = func_count (z-scored)", fontweight="bold")
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)

    ax = axes[1]
    parts = [uL, common, uM]
    labels = [f"unique to length\n(func_count)\n{uL:.3f}", f"shared\n{common:.3f}",
    ...
    ax.set_yticks([])
    ax.set_xlabel(f"R² beyond the model-type baseline (total = {r2[3] - r2[0]:.3f})",
                  fontweight="bold")
...
def directional_panelB_redraw(df: pd.DataFrame, outdir: Path) -> None:
    ...
    cell = lambda u, dn: tr.loc[(up_hi == u) & (down_hi == dn), "val_loss_step_AOC"]  # noqa: E731
    ...
    for k in means:
        print(f"  upward>{0.5}={str(k[0]):5s} downward>0.5={str(k[1]):5s}  mean AUC={means[k]:7.0f}  n={ns[k]}")
    ...
    for bars, nn in [(b1, n_lo_up), (b2, n_hi_up)]:
        for rect, n in zip(bars, nn):
            ax.text(rect.get_x() + rect.get_width() / 2, rect.get_height() + 40,
                    f"{rect.get_height():,.0f}", ha="center", fontsize=11, fontweight="bold")
            ax.text(rect.get_x() + rect.get_width() / 2, rect.get_height() / 2,
                    f"n = {n:,}", ha="center", va="center", fontsize=9,
                    color="white", fontweight="bold")
    ax.set_xticks(x)
    ax.set_xticklabels(["DOWNWARD-monotone runs\n(downward degree > 0.5)",
                        "not downward-monotone\n(downward degree ≤ 0.5)"], fontsize=11)
    ax.set_ylabel("Mean Validation Loss AUC   (lower = easier to learn)", fontweight="bold")
    ax.legend(title="Upward monotonicity", fontsize=10, title_fontsize=10)
    ax.grid(True, axis="y", linestyle="--", alpha=0.4)
    ax.set_title("Downward monotonicity drives ease of learning; upward adds little",
...
def directional_diagnostics(df: pd.DataFrame) -> None:
    ...
    print(f"{'degree':<16}{'mean':>7}{'sd':>7}{'median':>8}{'% == 0':>8}{'% >= .99':>10}")
    for c in DIRS + ["upward", "downward", "degree"]:
        print(f"{c:<16}{u[c].mean():>7.3f}{u[c].std():>7.3f}{u[c].median():>8.3f}"
              f"{(u[c] <= 0.001).mean() * 100:>7.1f}%{(u[c] >= 0.99).mean() * 100:>9.1f}%")
    print(f"\ncorr(upward, downward) = {stats.pearsonr(u['upward'], u['downward'])[0]:+.3f}")
    print(f"degree attained by downward sense: {(u['downward'] >= u['upward']).mean() * 100:.1f}% of expressions")
    ...
    mi = smf.ols("val_loss_step_AOC ~ (upward + downward) * C(model) + func_count",
                 data=tr).fit()
    for t in ["upward", "downward",
              "upward:C(model)[T.Transformer]", "downward:C(model)[T.Transformer]"]:
        print(f"  {t:<38} beta={mi.params[t]:+8.0f}  p={mi.pvalues[t]:.2e}")
    print("=> the downward advantage holds in both architectures and is larger for the Transformer.")
...
def operator_prevalence(df: pd.DataFrame) -> None:
    ...
    print(f"{'operator':<14}{'% expr attested':>16}{'total (uniq expr)':>18}"
          f"{'mean/expr':>11}{'total (trained runs)':>21}")
    for op in OPERATORS:
        cu = uniq[f"op_{op}"]
        ct = tr[f"op_{op}"]
        print(f"{op:<14}{(cu > 0).mean() * 100:>15.1f}%{cu.sum():>18d}"
...
def polarity_counterbalance(df: pd.DataFrame) -> None:
    ...
    for op in OPERATORS:
        c = u[f"op_{op}"]
        print(f"{op:<14}{stats.pearsonr(c, u['upward'])[0]:>+8.3f}"
    ...
    print(f"gap among difference==0 (n={m0.sum()}): {gap[m0].mean():.3f}"
          "   -> difference does NOT drive the surplus")
    print("\nmean degrees by `not` count (negation is the polarity flipper):")
    print(u.groupby(u["op_not"].clip(upper=2))[["upward", "downward"]]
...
def operator_by_architecture(df: pd.DataFrame, outdir: Path, n_boot: int = 500) -> None:
    ...
    tr = df[df["training"] == True].dropna(subset=["val_loss_step_AOC"]).copy()  # noqa: E712
    ...
    for arch, g in tr.groupby("model"):
        ...
        res[arch] = dict(coef=base.coef_, boot=boot, n=len(y),
                         r2=r2_score(y, base.predict(X)),
                         ymean=y.mean(), ysd=y.std())
    ...
    print(f"LSTM        n={L['n']}  mean AUC={L['ymean']:7.0f}  SD={L['ysd']:7.0f}  R^2={L['r2']:.3f}")
    print(f"Transformer n={T['n']}  mean AUC={T['ymean']:7.0f}  SD={T['ysd']:7.0f}  R^2={T['r2']:.3f}")
    ...
    for i in order:
        ...
        print(f"{OPERATORS[i]:<14}{L['coef'][i]:>8.0f}{T['coef'][i]:>8.0f}"
    ...
    ax = axes[0]
    for k, (r, col, lab) in enumerate([(L, "#E69F00", "LSTM"), (T, "#56B4E9", "Transformer")]):
        c = np.array(r["coef"])[order]
        lo, hi = np.percentile(r["boot"], [2.5, 97.5], axis=0)
        lo, hi = lo[order], hi[order]
        yy = y_pos + (off if k else -off)
        ax.errorbar(c, yy, xerr=[c - lo, hi - c], fmt="o", color=col, ecolor=col,
                    capsize=3, ms=6, lw=1.4, label=lab)
    ax.axvline(0, color="black", ls="--", lw=1)
    ax.set_yticks(y_pos)
    ax.set_yticklabels([OPERATORS[i] for i in order])
    ax.set_xlabel("Δ AUC per +1 SD of operator count\n(absolute units — inflated by the\n"
                  "Transformer's larger overall loss)", fontweight="bold")
    ...
    ax.errorbar(c, y_pos, xerr=[c - lo, hi - c], fmt="none", ecolor="gray", capsize=3, zorder=1)
    ax.scatter(c, y_pos, color=cols, s=55, zorder=2)
    for ci, yi, s in zip(c, y_pos, sig):
        if s:
            ax.text(ci, yi + 0.32, f"{ci:+.0f}", ha="center", fontsize=9, fontweight="bold")
    ax.axvline(0, color="black", ls="--", lw=1)
    ax.set_yticks(y_pos)
    ax.set_yticklabels([])
    ax.set_xlabel("Transformer − LSTM (absolute)\ncolour = 95% CI excludes 0", fontweight="bold")
    ...
    ax.set_yticks(y_pos)
    ax.set_yticklabels([])
    ax.set_xlabel("Δ AUC in WITHIN-architecture SD units\n(coefficient ÷ that architecture's AUC SD:\n"
                  "removes the overall-scale difference)", fontweight="bold")
    ax.set_title("(c) Rescaled — profiles nearly coincide", fontweight="bold")
    ax.legend(loc="lower right")
    ax.grid(True, axis="x", ls="--", alpha=0.4)

    fig.suptitle(f"Per-operator difficulty by architecture — same profile (r = {prof_r:.3f}), "
...
def paper_figure1_redl(df: pd.DataFrame, outdir: Path) -> None:
    ...
    try:
        from plotnine import (aes, element_line, element_rect, element_text,
                              geom_jitter, ggplot, guide_legend, guides, labs,
                              stat_smooth, theme, theme_minimal)
    except ImportError:
    ...
    plot = (
        ...
        + stat_smooth(method="lm", color="red", linetype="dashed", se=False,
                      size=1.2, fullrange=True)
...
def figure_vs_mixedmodel(df: pd.DataFrame, outdir: Path) -> None:
    ...
    """
    d = df[df["training"] == True].dropna(  # noqa: E712
        subset=["val_loss_step_AOC", "degree"]).copy()
    m = smf.mixedlm("val_loss_step_AOC ~ degree * C(model)", d,
                    groups=d["expression"], re_formula="~1").fit(reml=False)
    b = m.params
    grid = np.linspace(0, 1, 50)
    pred = {
        "LSTM": b["Intercept"] + b["degree"] * grid,
        "Transformer": (b["Intercept"] + b["C(model)[T.Transformer]"]
    ...
    for name, sub in d.groupby("model"):
        ax.scatter(sub["val_loss_step_AOC"], sub["degree"], s=8, alpha=0.35,
                   color=colors[str(name)], edgecolors="none", label=str(name))
    for name, line_col in [("LSTM", "#b03a2e"), ("Transformer", "#0e6655")]:
        ax.plot(pred[name], grid, color=line_col, lw=3,
                label=f"{name}  mixed-model fit")
    ...
    ax.set_title(f"paper_figure1.png scatter + mixed-effects model fits (same {len(d)} rows)",
                 fontsize=13)
...
def monotonicity_gap_analysis(df: pd.DataFrame, outdir: Path) -> None:
    ...
    labels = ["(0,.05]", "(.05,.10]", "(.10,.15]", "(.15,.20]", "(.20,.30]",
              "(.30,.50]", "(.50,.70]", "(.70,.90]", "(.90,.99)", "[.99,1]"]
    cols = ["degree"] + DIRS
    ...
    for lo, hi, lab in zip(bins[:-1], bins[1:], labels):
        print(f"{lab:<12}" + "".join(f"{((u[c] > lo) & (u[c] <= hi)).sum():>11d}" for c in cols))
    ...
    fams: dict[str, list[float]] = {"noisy": [], "threshold": [], "negated_thr": [],
                                    "equality": [], "bool2": []}
    for _ in range(1500):
        ...
        seed[gens] = True
        q = up_closure(seed) ^ (rng.random(n_models) < rng.choice([0.05, 0.1, 0.2, 0.35, 0.5]))
        if 0.02 < q.mean() < 0.98:
        ...
        for name, q in [("threshold", c1 >= k), ("negated_thr", ~(c1 >= k)),
                        ("equality", c1 == k)]:
            if 0.02 < q.mean() < 0.98:
        ...
        if s2.sum():
            ...
            q = rng.choice([np.logical_and, np.logical_or, np.logical_xor])(c1 >= k, ~(c2 >= k2))
            if 0.02 < q.mean() < 0.98:
                fams["bool2"].append(mon_up(q))

    cats = [("== 0", lambda v: v <= 1e-4), ("(0, .15]", lambda v: (v > 1e-4) & (v <= 0.15)),
            ("(.15, .99)", lambda v: (v > 0.15) & (v < 0.99)), ("[.99, 1]", lambda v: v >= 0.99)]
    ...
    print("  -> noisy quantifiers DO fill (0,.15]; among crisp families only "
    ...
    ax.bar(edges[:-1] + 0.0125, counts, width=0.023, color="#4C72B0", label="degree > 0")
    ax.bar([0.0], [(deg <= 1e-4).sum()], width=0.023, color="#1a1a2e", label="exactly 0")
    ax.axvspan(0.001, 0.15, color="#c0392b", alpha=0.12)
    ax.text(0.075, ax.get_ylim()[1] * 0.05 + max(counts) * 0.9, "the gap:\n34 exprs",
    ...
    ax.set_title("(a) Real data: mass at 0, a sparse zone to ~0.15, then a continuum",
