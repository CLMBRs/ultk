...
def graph_expression(expression:Expression, name:str, current_dir:str, show:bool=False):
    ...
    for ref in referents:
        if(expression.meaning._dist is not None):
            plt.scatter(ord(ref.V)-64, int(ref.H), s=expression.meaning._dist[ref.name]*10)
        else:
...
def graph_language_color_distribution(language:ColorLanguage, current_dir:str):
    ...
    for form, centroid in form_centroid_map.items():
        ...
        rgb_hex = '#%02x%02x%02x' % (int(rgb_trio[0]*255), int(rgb_trio[1]*255), int(rgb_trio[2]*255))
    ...
    for major_term in language.expressions:
        ...
        for referent in major_term.meaning.referents:
            if referent.name in most_probable_referents:
                if major_term.meaning._dist[referent.name] > most_probable_referents[referent.name][1]:
                    most_probable_referents[referent.name] = (form, major_term.meaning._dist[referent.name])
            else:
                most_probable_referents[referent.name] = (form, major_term.meaning._dist[referent.name])

    for referent, (form, prob) in most_probable_referents.items():
        language_color_data.append((form, ord(referent[0])-96, int(referent[1:]), prob, rgb_centroid_map))
    #Get the centroid color of the most probable expression associated with each referent
    language_color_data = pd.DataFrame(language_color_data, columns =['form','V','H', 'prob', 'rgb_centroid'])
    plot = (
