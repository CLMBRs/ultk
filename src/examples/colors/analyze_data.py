...
from graph_colors import (
    graph_complexity,
    graph_expression,
    graph_language_color_distribution,
)
import pandas
import util
...
def main(args):
    ...
    languages, artificial_languages, meaning_dist, prior = generate_color_languages(
        num_languages=args.number_of_languages,
        color_chip_threshold=args.color_chip_threshold,
    )
    ...
    generate_color_complexity(
        nat_langs=languages,
        art_langs=artificial_languages,
        meaning_dist=meaning_dist,
        prior=prior,
        use_rkk=args.use_rkk,
        generate_ib_bound=args.generate_ib_bound,
    )
    ...
    complexity_dataframe: DataFrame = pandas.read_csv(
        f"{current_dir}/outputs/complexity_data.csv"
    )
    graph_complexity(
        complexity_dataframe,
        ib_bound,
        current_dir,
        show_labels=args.generate_graph_language_labels,
    )
