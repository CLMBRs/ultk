...
def generate_color_languages(num_languages = -1, 
    ...
    for language_code, language_colors in language_colors_to_expressions.items():
        ...
        for color in language_colors:
            ...
            most_frequent_color_term = language_colors_to_expressions[language_code][color].most_common(1)[0][0]
        ...
        for major_term_index, major_term in enumerate(major_color_terms):
            ...
            major_term_meaning = HashableMeaning(referents=color_referent_tuple,
                                        universe=color_universe,
                                        _dist=dict(zip([referent.name for referent in color_referent_tuple], expression_meanings[major_term_index])))
    ...
    artificial_languages = [ColorLanguage(language.expressions, is_natural=False) for language in sampling.random_languages(expressions=expression_list, sampling_strategy="stratified", sample_size=20, max_size=15)]
