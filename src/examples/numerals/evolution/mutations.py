...
from ..numerals_language import NumeralsLanguage, is_D_rule, is_M_rule, numerals_universe
...
# Helpers
def remove_rule(grammar: Grammar, filter: Callable[[Rule], bool],) -> Grammar:
...
def add_rule(grammar: Grammar, filter: Callable[[Rule], bool], lhs: Any, postfix: str,) -> Grammar:
    ...
    rule_to_add = Rule(
        f"_{name_to_add}_{postfix}", lhs, None, func=lambda _: name_to_add,
    )
...
class GrammarMutation(Mutation):
    ...
    @staticmethod
    @abstractmethod
    def mutate(language: NumeralsLanguage, grammars: set[Grammar], **kwargs) -> NumeralsLanguage:
...
class RemoveRule(GrammarMutation):
    @staticmethod
    def mutate(language: NumeralsLanguage, grammars: set[Grammar], category: str, **kwargs) -> NumeralsLanguage:
        ...
        for _ in range(kwargs.get("max_attempts", MAX_ATTEMPTS)):
            new_grammar = remove_rule(
                language.grammar,
                is_D_rule if category == "D" else is_M_rule
            )
...
class AddRule(GrammarMutation):
    ...
    def mutate(language: NumeralsLanguage, grammars: set[Grammar], category: str, **kwargs) -> NumeralsLanguage:
        for _ in range(kwargs.get("max_attempts", MAX_ATTEMPTS)):
            args = (is_D_rule, Number, "D") if category == "D" else (is_M_rule, Multiplier, "M")
