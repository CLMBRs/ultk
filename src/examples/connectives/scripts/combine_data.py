...
if __name__ == "__main__":
    ...
    dominating_languages = yaml_to_dataframe(
        "connectives/outputs/dominating_languages.yml", keys
    )
    explored_languages = yaml_to_dataframe(
        "connectives/outputs/explored_languages.yml", keys
    )
    all_data = pd.concat(
        [
            explored_languages,
    ...
    all_data.to_csv("connectives/outputs/combined_data.csv", index=False)
