...
class IBLanguage:
    ...
    def __init__(self, structure: IBStructure, expressions: tuple[FrozenDict[Meaning, float], ...]):
    ...
    def qwm(self) -> np.ndarray:
        return np.array(
            [
                [expression[meaning] for meaning in self.structure.meanings] for expression in self.expressions
            ]
        )
    ...
    def qmw(self) -> np.ndarray:
        # Apply Bayes' rule
        return (self.qwm.transpose() * self.structure.meanings_prior[:,None] / self.expressions_prior)
    
    # TODO: CHECK THIS MORE EXTENSIVELY
    @cached_property
    def complexity(self) -> float:
        return np.sum(safe_log(self.qwm / self.expressions_prior[:,None]) * (self.qwm * self.structure.meanings_prior))
    ...
    def expected_divergence(self):
        ...
        right = np.array([
            [kl_divergence(k, r) for k in self.structure.mu.T] for r in self.reconstructed_meanings.T
        ])
        return np.sum(left * right)

    # I(W; U): Accuracy of the lexicon
    # TODO: Check this extensively
    # TODO: The math behind this needs to be redone because this is just complexity
    @cached_property
    def iwu(self) -> float:
        ref_to_expr = self.qwm @ self.structure.mu_inverse
        A = safe_log(ref_to_expr / self.expressions_prior[:,None])
        return np.sum(A * (ref_to_expr * self.structure.referents_prior))
