...
class IBStructure:
    ...
    def __init__(
        ...
        for meaning in meanings:
            for referent in referents:
                if referent not in meaning:
                    raise ValueError("Meanings provided are not compatible with referents")
        ...
        object.__setattr__(
            self, "meanings_prior", np.array(prior or [1 / len(meanings) for _ in meanings])
        )
        object.__setattr__(
    ...
    def mu(self) -> np.ndarray:
        ...
        return np.array(
            [
                [meaning.mapping[referent] for referent in self.referents] for meaning in self.meanings
            ]
        )
